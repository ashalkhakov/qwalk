...
typedef struct mdl_simpleframe_s
{
	mdl_trivertx_t min;
	mdl_trivertx_t max;
	char name[16];
	mdl_trivertx_t *frame;
} mdl_simpleframe_t;
...
static const unsigned char quakepalette[768] = 
{
...
/* read frames */
	frames = (mdl_simpleframe_t*)qmalloc(sizeof(mdl_simpleframe_t) * header->numframes);
	for (i = 0; i < header->numframes; i++)
	{
		...
		if (type == 0)
		{
			mdl_simpleframe_t *sframe = (mdl_simpleframe_t*)f;
			/* mdl_simpleframe_t contains only bytes, so no swapping needed */

			frames[i].min = sframe->min;
			frames[i].max = sframe->max;
			strcpy(frames[i].name, sframe->name);

			/* advance the size of the struct minus a memory pointer (not present in the disk format) */
			f += sizeof(mdl_simpleframe_t) - sizeof(frames[i].frame);

			/* frames[i].frame points to a list of trivertxs (vertex data in compressed format) */
			frames[i].frame = (mdl_trivertx_t*)f;
			f += sizeof(mdl_trivertx_t) * header->numverts;
		}
		else if (type == 1)
		{
		/* framegroup */
...
/* load into a real model */
	...
	for (i = 0; i < header->numframes; i++)
	{
		model.frameinfo[i].name = (char*)qmalloc(strlen(frames[i].name) + 1);
		strcpy(model.frameinfo[i].name, frames[i].name);
	}
	...
	for (i = 0; i < model.num_frames; i++)
	{
		...
		{
			const mdl_trivertx_t *trivertx = &frames[i].frame[meshverts[j].vertex];
...
/* calculate bounds */
	...
	for (i = 0; i < model->num_frames * mesh->num_vertices; i++)
	{
		float *xyz = mesh->vertex3f + i * 3;
...
/* calculate the totally inane "average polygon size" value */
	totalsize = 0.0f;
	for (i = 0; i < mesh->num_triangles; i++)
	{
...
/* write frames */
	for (i = 0; i < model->num_frames; i++)
	{
		mdl_simpleframe_t *simpleframe;

		*(int*)f = 0; /* frametype (FIXME) */
		f += sizeof(int);

		simpleframe = (mdl_simpleframe_t*)f;

		simpleframe->min.packedposition[0] = 0; /* these will be set in the following loop */
		simpleframe->min.packedposition[1] = 0;
		simpleframe->min.packedposition[2] = 0;
		simpleframe->min.lightnormalindex = 0;
		simpleframe->max.packedposition[0] = 0;
		simpleframe->max.packedposition[1] = 0;
		simpleframe->max.packedposition[2] = 0;
		simpleframe->max.lightnormalindex = 0;
		strlcpy(simpleframe->name, model->frameinfo[i].name, sizeof(simpleframe->name));

		f += sizeof(mdl_simpleframe_t) - sizeof(simpleframe->frame);

		for (j = 0; j < mesh->num_vertices; j++)
		{
			float pos[3];
			mdl_trivertx_t *trivertx = (mdl_trivertx_t*)f;

			pos[0] = (mesh->vertex3f[i*mesh->num_vertices*3+j*3+0] - header->origin[0]) / header->scale[0];
			pos[1] = (mesh->vertex3f[i*mesh->num_vertices*3+j*3+1] - header->origin[1]) / header->scale[1];
			pos[2] = (mesh->vertex3f[i*mesh->num_vertices*3+j*3+2] - header->origin[2]) / header->scale[2];
			...
			trivertx->lightnormalindex = compress_normal(mesh->normal3f+i*mesh->num_vertices*3+j*3);

			if (trivertx->packedposition[0] < simpleframe->min.packedposition[0] || j == 0)
				simpleframe->min.packedposition[0] = trivertx->packedposition[0];
			if (trivertx->packedposition[1] < simpleframe->min.packedposition[1] || j == 0)
				simpleframe->min.packedposition[1] = trivertx->packedposition[1];
			if (trivertx->packedposition[2] < simpleframe->min.packedposition[2] || j == 0)
				simpleframe->min.packedposition[2] = trivertx->packedposition[2];

			if (trivertx->packedposition[0] > simpleframe->max.packedposition[0] || j == 0)
				simpleframe->max.packedposition[0] = trivertx->packedposition[0];
			if (trivertx->packedposition[1] > simpleframe->max.packedposition[1] || j == 0)
				simpleframe->max.packedposition[1] = trivertx->packedposition[1];
			if (trivertx->packedposition[2] > simpleframe->max.packedposition[2] || j == 0)
				simpleframe->max.packedposition[2] = trivertx->packedposition[2];

			f += sizeof(mdl_trivertx_t);
		}
	}
